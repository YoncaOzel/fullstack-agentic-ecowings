    ...
    public class AccountService : IAccountService
    {
        ...
        public async Task<string> RegisterAsync(RegisterRequest request, string origin)
        {
            ...
            if (userWithSameEmail == null)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var customUser = new CleanArchitecture.Core.Entities.User
                    {
                        Name = $"{request.FirstName} {request.LastName}",
                        Email = request.Email,
                        PasswordHash = "ManagedByIdentity", // Şifre zaten Identity sisteminde korunduğu için buraya formel bir metin yazabiliriz
                        Role = CleanArchitecture.Core.Enums.UserRoles.Customer,
                        DateOfBirth = DateTime.UtcNow, // Şu an formda olmadığı için varsayılan değer
                ...
                else
                {
                    ...
                    throw new ApiException($"Kayıt Başarısız: {errors}");
                }
            }
            else
            {
        ...
        private async Task<JwtSecurityToken> GenerateJWToken(ApplicationUser user)
        {
            ...
            var claims = new[]
            {
                ...
                new Claim("customUserId", customUserId),
        ...
        public async Task<EmailRequest> ForgotPassword(ForgotPasswordRequest model, string origin)
        {
            ...
            var encodedToken = Uri.EscapeDataString(code);
            var encodedEmail = Uri.EscapeDataString(model.Email);
            var resetLink = $"{origin}/reset-password?email={encodedEmail}&token={encodedToken}";
            var emailRequest = new EmailRequest()
            {
                Body = $"<p>Hi {account.FirstName ?? account.UserName},</p><p>Click the link below to reset your Eco-Wings password. This link expires in 60 minutes.</p><p><a href=\"{resetLink}\">Reset My Password</a></p><p>If you did not request this, you can safely ignore this email.</p>",
                To = model.Email,
